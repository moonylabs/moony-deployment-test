...
const (
	accountTableName         = "ocp__core_vmmemoryaccount"
	allocatedMemoryTableName = "ocp__core_vmmemoryallocatedmemory"
)
...
func fromAccountModel(obj *accountModel) *ram.Record {
	return &ram.Record{
		...
		StoredAccountType: vm.VirtualAccountType(obj.StoredAccountType),
...
func dbReserveMemory(ctx context.Context, db *sqlx.DB, vm string, accountType vm.VirtualAccountType, address string) (string, uint16, error) {

...
func (s *store) ReserveMemory(_ context.Context, vm string, accountType vm.VirtualAccountType, address string) (string, uint16, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *store) findByVmAndAccountType(vm string, accountType vm.VirtualAccountType) []*ram.Record {
	var res []*ram.Record
	for _, item := range s.records {
		if item.Vm != vm {
			continue

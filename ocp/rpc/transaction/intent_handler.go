...
func NewOpenAccountsIntentHandler(conf *conf, log *zap.Logger, data ocp_data.Provider, antispamGuard *antispam.Guard) CreateIntentHandler {
...
func validateClaimedGiftCard(ctx context.Context, data ocp_data.Provider, giftCardVaultAccount *common.Account, claimedAmount uint64) error {
	//
	...
	if time.Since(accountInfoRecord.CreatedAt) >= account_worker.GiftCardExpiry-time.Minute {
		return NewStaleStateError("gift card is expired")
	}
...
func validateDistributedPool(ctx context.Context, data ocp_data.Provider, poolVaultAccount *common.Account, distributedAmount uint64) error {
	//
...
func validateTimelockUnlockStateDoesntExist(ctx context.Context, data ocp_data.Provider, openAction *transactionpb.OpenAccountAction) error {
...
func getExpectedTimelockVaultFromProtoAccounts(ctx context.Context, data ocp_data.Provider, authorityProto, mintProto *commonpb.SolanaAccountId) (*common.Account, error) {

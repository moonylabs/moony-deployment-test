...
func (p *runtime) markAvailable(ctx context.Context, record *nonce.Record) error {
	// We now know the blockhash for the nonce.
	record.State = nonce.StateAvailable
	return p.data.SaveNonce(ctx, record)
}

func (p *runtime) markInvalid(ctx context.Context, record *nonce.Record) error {
	// We failed to create the nonce account (insufficient funds, etc).
	record.State = nonce.StateInvalid
	return p.data.SaveNonce(ctx, record)
}

func (p *runtime) sign(tx *solana.Transaction, key *vault.Record) error {
...
func (p *runtime) getLatestBlockhash(ctx context.Context) (*solana.Blockhash, error) {
...
func (p *runtime) getTransaction(ctx context.Context, signature string) (*solana.ConfirmedTransaction, error) {
	return p.getTransactionFromBlockchain(ctx, signature)
}

func (p *runtime) getTransactionFromBlockchain(ctx context.Context, signature string) (*solana.ConfirmedTransaction, error) {
...
func (p *runtime) getRentAmount(ctx context.Context) (uint64, error) {
	if p.rent > 0 {
		return p.rent, nil
	}
...
func (p *runtime) createSolanaMainnetNonce(ctx context.Context, purpose nonce.Purpose) (*nonce.Record, error) {
...
func (p *runtime) createNonceAccountTx(ctx context.Context, nonce *nonce.Record) (*solana.Transaction, error) {
...
func (p *runtime) broadcastTx(ctx context.Context, tx *solana.Transaction) {

	...
	for {
		...
		case <-time.After(delay):
			...
			for _, state := range []nonce.State{
				...
				count, err := p.data.GetNonceCountByStateAndPurpose(ctx, nonce.EnvironmentVm, common.CoreMintVmAccount.PublicKey().ToBase58(), state, nonce.PurposeClientIntent)
				if err != nil {
					continue
				}
				recordNonceCountEvent(ctx, nonce.EnvironmentVm, common.CoreMintVmAccount.PublicKey().ToBase58(), state, nonce.PurposeClientIntent, count)
